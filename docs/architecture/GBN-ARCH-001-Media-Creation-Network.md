...
```mermaid
graph TB
    subgraph MCNClient["MCN Client (Creator's Device)"]
        Capture["Video Capture\n/ Import"]
        Sanitizer["Metadata Stripper\n+ Visual Anonymizer"]
        Chunker["Chunker\n(1MB default)"]
        Encryptor["Per-Chunk Encryptor\n(AES-256-GCM)"]
        OnionBuilder["Onion Packet\nBuilder"]
        CircuitMgr["Circuit Manager\n(multi-path)"]
        KeyMgr["Key Manager\n(ephemeral ECDH)"]
        AckReceiver["Ack Receiver\n& Progress UI"]
    end

    subgraph BON["Broadcast Overlay Network"]
        Relay1["Relay Node 1\n(guard)"]
        Relay2["Relay Node 2\n(middle)"]
        Relay3["Relay Node 3\n(exit)"]
    end

    subgraph Publisher["Publisher (MPub)"]
    ...
    KeyMgr --> Encryptor
    Chunker --> Encryptor
    Encryptor --> OnionBuilder
    OnionBuilder --> CircuitMgr
    CircuitMgr --> Relay1
    Relay1 --> Relay2
    Relay2 --> Relay3
    Relay3 --> ChunkReceiver
    ChunkReceiver -.->|"signed ack (via BON)"| AckReceiver
```
